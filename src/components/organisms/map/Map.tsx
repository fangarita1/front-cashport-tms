...
const MapComponent = () => {
  ...
  const [trips, setTrips] = useState<ISocketTrip[]>([])
  ...
  const updateUserLocation = (data: ISocketData) => {
    ...
    setTrips((prevSocketInfo) => {
      return [...prevSocketInfo, data.trip];
    })
        ...
        if (mapRef.current) {
          const popup = new mapboxgl.Popup({
            ...
            <div style="display:flex;justify-content:space-between">
                ...
                <div style="width:51px;height:51px;background-color:#fff;border-radius:6px"></div>
                <div>
                  <div style="display:flex;align-items:center;column-gap:15px">
                    <div style="color:#fff;font-size:18px;font-style:normal;font-weight:700;line-height:24px;letter-spacing:-.5px">Camion C-100</div>
                    <div style="padding:2px 8px;border-radius:7px;color:#677fb0;font-size:14px;font-weight:600;line-height:20px;background-color:#2e323a">CHM-124</div>
                    </div>
                    ...
                    <div style="display:flex;column-gap:17px;">
                      ...
                      <div>
                      <div style="color:#fff;font-size:14px;font-weight:600;line-height:20px">Base Barrancabermeja</div>
                      <div style="color:#fff;font-size:14px;font-weight:400;line-height:20px">17 Mar. 2024 - 11:30 a.m</div>
                      </div>
                    </div>
                    <div style="display:flex;column-gap:17px;">
                      <div>
                      ...
                      <div>
                      <div style="color:#6f7a90;font-size:14px;font-weight:600;line-height:20px">Centro Empresarial Dorado</div>
                      <div style="color:#6f7a90;font-size:14px;font-weight:400;line-height:20px">17 Mar 2024 - 5:30 p.m</div>
                      </div>
                    </div>
                    </div>
                  </div>
              ...
              {showCards && (
                <div className={styles.cardContainer}>
                  {trips.map((item, index) => (
                    <div className={`${styles.card} ${index !== 7 && styles.bottomDivider}`} key={item.id}>
                      <div className={styles.leftSection}>
                        <div className={styles.img} />
                        <div>
                          <div className={styles.cardTitleContainer}>
                            <div className={styles.cardTitle}>{item.vehicle.behicleType}</div>
                            <div className={styles.cardSubtitle}>{item.vehicle.plateNumber}</div>
                          </div>
                      ...
                      <div className={styles.rightSection}>
                        <div className={styles.rightTitle}>TR 334455</div>
                        <div className={styles.stateTag}>
                          <CheckCircle size={16} color='#141414' />
                          <div className={styles.state}>En curso</div>
                        </div>
