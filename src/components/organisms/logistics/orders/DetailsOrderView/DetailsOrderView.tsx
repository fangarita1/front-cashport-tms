...
export const DetailsOrderView = ({ idOrder = "" }: Props) => {
  ...
  const { routeGeometry, distance, timetravel, mapContainerRef } = useMapbox({
    start_longitude: transferOrder?.start_location?.longitude,
    start_latitude: transferOrder?.start_location?.latitude,
    end_longitude: transferOrder?.end_location?.longitude,
    end_latitude: transferOrder?.end_location?.latitude,
              ...
              <Col span={12}>
                <Flex className={styles.sectionContainer}>
                  <SummaryData
                    ...
                    start_date_flexible={optionsFlexible.find(x=> x.value==transferOrder?.start_date_flexible)?.label ?? ""}
                    end_date_flexible={optionsFlexible.find(x=> x.value==transferOrder?.end_date_flexible)?.label ?? ""}
